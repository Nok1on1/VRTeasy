...
public class VRTeasy extends VRTBase {
    ...
    public List<TestRunResponse> downloadAndTrackPDF(String xpath, TestRunStatus expectedStatus) {
        Path filePath = vrtClient.downloadPDF(null, xpath);
        return trackPDF(filePath, expectedStatus);
    }


    public List<TestRunResponse> downloadAndTrackPDF(String fileName, String xpath, TestRunStatus expectedStatus) {
        Path filePath = vrtClient.downloadPDF(fileName, xpath);
        return trackPDF(filePath, expectedStatus);
    }
}
